...
const CreateOrderModal: React.FC<CreateOrderModalProps> = ({ isOpen, isCancel, customerid = null }) => {
  ...
  const handleSubmit = async (data: FormData) => {
    try {
      const { orderNo, date, note, items } = data;
      ...
      const measurementPromises = items?.map((item: any) => {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: `Failed to create order and items: ${error instanceof Error ? error.message : 'Unknown error'}`,
      });
